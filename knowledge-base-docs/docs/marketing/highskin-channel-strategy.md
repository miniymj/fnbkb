...
| **PAID** | 인스타/네이버 쇼핑 광고 · 인플루언서 협찬 | **Always-On 소액 상시 + 시즌 부스터** 이원 운영 (§3-1) — 초기엔 PAID 비중 최소화 |
...
| **AI 어시스턴트** (ChatGPT·네이버 AI쇼핑) | "민감피부 아이 샴푸바 추천줘" 자연어 질문 | 높음 — 상품 피드 품질로 무료 진입 가능 | **간접 대응** |
| 유튜브/쇼츠 | 클린뷰티·리빙 롱폼 리서치 | 보통 | 향후 검토 |
| 틱톡 | 발견성 높은 숏폼 | 보통 | 향후 검토 |

> **AI 어시스턴트 채널 전략** (Criteo×OpenAI 인사이트): 경쟁 입찰 없이 **관련성 기반** 노출 → 광고비가 아니라 상품명·카테고리·속성·후기 품질로 진입한다. 별도 채널 운영이 아니라 **스토어 상품 정보의 GEO 최적화**(`launch-playbook.md` §2 체크리스트)로 대응. 분기 1회 ChatGPT 셀프 테스트로 언급 여부 점검. → `ai-performance-insights.md` §5·§8
## 3. 크로스채널 퍼널 — 유입 → 신뢰 → 전환
...
|---|---|---|---|
| 1단계 유입(인지) | 인스타 릴스 · 픽커 퀴즈 공유 · **AI 어시스턴트 추천(간접)** | 브랜드를 처음 만남 | 캐릭터 표정 릴스, "나만의 하이피부 친구" 퀴즈, 상품명·속성 GEO 최적화 |
| 2단계 신뢰(고려) | 네이버 블로그 · 인스타 피드 · **ChatGPT 답변에 인용될 정보성 글** | 구매 전 리서치 중 | 민감피부 성분 가이드, 리필 시스템 설명, 리뷰 |
| 3단계 전환(구매) | 스마트스토어 · 쇼핑광고(스폰서) | 구매 결정 직전 | 상세페이지(소재·사이즈) + 리뷰 + 픽커 결과 상품 연결 |
| 4단계 충성(재구매) | 정기구독 · DM | 리필 시점 | 리필 쿠폰, 새 향·한정판 안내 |

> **비주얼 가이드라인 일관 적용**: 모든 단계에서 7색 팔레트 + 손그림 캐릭터 + 라운드 폰트 유지 → "어느 채널에서 만나도 하이피부".

### 3-1. 광고 운영 모델 — Always-On + 시즌 부스터 (Criteo GO 베끼기)

- **Always-On 베이스**: 소액 자동화 상시 운영 — 월별 이슈 캘린더 priority Mid/Low 달. 리필 구독 인지 유지용
- **시즌 부스터**: priority High 달(설·가정의달·여름휴가·추석·연말) — 예산 상향 + 수동 정밀 타겟팅 + 시즌 크리에이티브(월별 카드뉴스 E2c)
- **유기적 vs 스폰서 이원화**: 블로그·인스타 = 유기적 신뢰 / 쇼핑광고 = 스폰서 전환 — ChatGPT 광고 구조(organic 옆 sponsored)와 동일한 이원 체계
- 실행 프로토콜(목표 4종·학습기간·순차 테스트): `launch-playbook.md` §4 · 배경: `ai-performance-insights.md` §3~§4
...
- 광고·AI 채널: `marketing/ai-performance-insights.md` (Criteo GO·ChatGPT 광고 인사이트)
---