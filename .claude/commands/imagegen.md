...
## 실행
```bash
# 기본 (output/gen-image.png)
python3 scripts/gen-image.py "프롬프트"

# 경로·크기 지정
python3 scripts/gen-image.py "프롬프트" --out output/moodboard-highskin.png --size 1024x1536
```
...
4. **저장 위치**: 기본 `output/` (문서·스모어·스토어에 쓰일 이미지는 파일명을 브랜드/용도에 맞게 지정)
5. **결과 보고**: 생성된 파일의 **절대 경로**와 (있으면) revised_prompt를 응답에 포함
6. **비용 주의**: gpt-image-1은 이미지당 요금 — 대량 생성 전 사용자에게 확인
